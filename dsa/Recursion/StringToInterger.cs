...
// Reading stops at the first non-digit character 'w'.
    ...
    public static int MyAtoi(string s) {
        ...
        var res = Helper(s, startIndex, 0, isNagative);
        return (int)res;
    }
    private static long Helper(string s, int i, long total, bool isNegative)
    {
        if (i >= s.Length) return isNegative ?  total * -1 : total;
        if (char.IsDigit(s[i]))
        {
            int num = int.Parse(s[i].ToString());
            total = (total * 10) + num;
            if (total > int.MaxValue) return isNegative ? int.MinValue : int.MaxValue;;
            return Helper(s, i + 1, total, isNegative);
        }
        else return isNegative && total > 0 ?  total * -1 : total;
